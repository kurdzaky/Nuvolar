...
@Slf4j
    ...
    public static final String TEST_MESSAGE = "Test '";
        ...
        public void onTestSuccess(ITestResult tr) {
            ...
            logger.info("Priority of this method is {}", tr.getMethod()
                    .getPriority());
        }
        ...
        public void onFinish(ITestContext context) {
                ...
                for (ITestNGMethod method : methods) {
                    ...
                    nrFailures = rMapFailed.getResults(method)
                            .size();
                    nrPassed = context.getPassedTests()
                            .getResults(method)
                            .size();
                    message += "Number of failures: " + nrFailures + " of total executions "
                            + (nrFailures + nrPassed) + " for method " + method.getMethodName() + "";
                    ...
                    for (ITestResult tr : rMapFailed.getResults(method)) {
                        ...
                        errors.append(tr.getTestContext()
                                        .getAttribute(TEST_USER_ID) != null ? tr.getTestContext()
                                        .getAttribute(TEST_USER_ID)
                                        : "")
                                .append(" " + tr.getName() + " ")
                                .append(tr.getThrowable()
                                        .getMessage()
                                        + "\n");

                        errors = getStringBuilder(errors, tr);
                        trPrevious = tr;
                        className = tr.getTestClass()
                                .getName();
                    }
        ...
        private String getClassName(ITestNGMethod method, int nrFailures, IResultMap rMapPassed,
                                    String className) {
            if (nrFailures == 0) {
